...
export class Editor {
    ...
    private handleCanvasMousedown = (e: MouseEvent): void => {
        ...
        const startCol = logicalWidthToCol(this.state.visualColoff, targetLine.text);
        ...
        this.state.visualCol = calcLogicalWidth(targetLine.text.slice(0, targetCol));
        this.state.prefVisualCol = cursorLogiWidth;
    ...
    private vi_executeOperator(
        ...
        if (operator === OperatorName.DELETE || operator === OperatorName.CHANGE) {
            if (linewise) {
                ...
                if (this.state.visualCol >= calcLogicalWidth(this.currentLine.text)) {
                    this.moveCursorToLast();
                }
            } else {
                // カーソル位置を対象範囲の先頭に移動する
    ...
    private vi_executeNormal(input: readonly string[]): 0 | 1 | 2 {
        ...
        switch (datatype) {
            case NormalCmdType.MOTION: {
                this.vi_executeMotion(data.motion, count);

            } break;
            case NormalCmdType.GO_INSERT: {
                this.vi_executeInsert(data.command, count);

            } break;
            case NormalCmdType.OPERATOR: {
                ...
                if (!range) return 0;
                const isLinewise = data.motion.type === "linewise" || range.linewise; // dj/ykのようなmotiontypeはcharだがrangeとしては行単位の挙動を持つ場合がある
            ...
            case NormalCmdType.UNDO: {
                for (let i = 0; i < count; i++) this.undo();

            } break;
            case NormalCmdType.REDO: {
    ...
    private vi_executeVisual(input: readonly string[]): 0 | 1 | 2 {
        ...
        switch (datatype) {
            ...
            case VisualCmdType.JOIN: {
                this.moveCursorToPos(vi_state.visualFirst.row, vi_state.visualFirst.col);
                this.vi_executeJoin(vi_state.lineCount);
                this.vi_goNormal();

            } break;
            case VisualCmdType.REPLACE: {
    ...
    private moveToWidestLine(): void {
        ...
        const destCol = Math.min(
            maxWidthLine.size - 1,
            logicalWidthToCol(this.state.visualCol, maxWidthLine.text)
        );
        ...
        switch (key) {
            case MOVE_KEYS.LEFT: {
                if (this.state.col !== 0) {
                    ...
                    this.state.visualCol = calcLogicalWidth(prevLine.text);
            ...
            case MOVE_KEYS.DOWN: {
                if (this.state.row < this.state.lines.length - 1) {
                    ...
                    this.state.visualCol = calcLogicalWidth(text);
                }
                break;
            }
        }
    ...
    private deleteChar(): void {
        ...
        if (this.state.col > 0) {
            ...
            this.state.col--;
            this.state.visualCol -= calcLogicalWidth(targetChar);
        } else {
            // append two lines
            const prevLine = this.prevLine as Line;
    ...
    private moveCursorLeft(): void {
        ...
        this.state.col--;
        this.state.visualCol -= calcLogicalWidth(prevChar);
        this.syncPreferredWidth();
    }
    ...
    private moveCursorRight(): void {
        ...
        this.state.col++;
        this.state.visualCol += calcLogicalWidth(currChar);
        this.syncPreferredWidth();
    }
    ...
    private moveCursorUp(): void {
        ...
        if (this.state.col >= prevLine.size && !prevLine.isEmpty()) {
            ...
            this.state.col--;
            this.state.visualCol -= calcLogicalWidth(lastChar);
        }
    }
    ...
    private moveCursorToFirstNonWhitespace(): void {
        ...
        this.state.visualCol = calcLogicalWidth(line.text.slice(0, start));
        this.syncPreferredWidth();
    }
    ...
    private moveCursorToLast(): void {
        ...
        this.state.col = end;
        this.state.visualCol = calcLogicalWidth(line.text.slice(0, end));
        this.syncPreferredWidth();
    }
    ...
    private recalcColWidth(destLine: Line): void {
        const logicalWidth = Math.min(this.state.prefVisualCol, calcLogicalWidth(destLine.text));
        const col = logicalWidthToCol(logicalWidth, destLine.text);
        this.state.col = col;
        this.state.visualCol = calcLogicalWidth(destLine.text.slice(0, col));
    }
    ...
    private clampCursorCol(): void {
        ...
        if (!maybeChar) {
            ...
            this.state.col = col;
            this.state.visualCol = calcLogicalWidth(line.text.slice(0, col));
        }
    }
