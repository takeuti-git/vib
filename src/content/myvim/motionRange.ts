...
export function getMotionRange(
    ...
    switch (motion.type) {
        case "char": {
            switch (motion.name) {
                case MotionName.up: // fallthrough
                case MotionName.firstCharPrevLine:
                    linewise = true;
                    begin.row = Math.max(0, begin.row - count);
                    begin.col = 0;
                    end.col = Math.max(0, lines[end.row]!.size - 1);
                    break;
                case MotionName.left:
                    if (col === 0) return undefined;
                    begin.col = Math.max(0, begin.col - count);
                    end.col--;
                    break;
                case MotionName.down: // fallthrough
                case MotionName.firstCharNextLine:
                    linewise = true;
                    begin.col = 0;
                    end.row = Math.min(end.row + count, maxRow);
                    end.col = Math.max(0, lines[end.row]!.size - 1);
                    break;
                case MotionName.right:
                    end.col = Math.min(end.col + count - 1, currLine.size - 1);
                    end.col = Math.max(0, end.col);
                    break;
                case MotionName.firstChar:
                    begin.col = getCountUntilNonWhitespace(currLine.text);
                    end.col = Math.max(0, end.col - 1);
                    break;
                case MotionName.last:
                    end.col = Math.max(0, currLine.size - 1);
                    break;
                case MotionName.first:
                    begin.col = 0;
                    end.col = Math.max(0, end.col - 1);
                    break;
                case MotionName.firstLine:
                    linewise = true;
                    begin.row = 0;
                    begin.col = 0;
                    end.col = Math.max(0, lines[end.row]!.size - 1);
                    break;
                case MotionName.lastLine:
                    linewise = true;
                    begin.col = 0;
                    end.row = state.lines.length - 1;
                    end.col = Math.max(0, lines[end.row]!.size - 1);
                    break;
                case MotionName.word_forward: // fall through
                case MotionName.WORD_forward: {
                    const sep = motion.name === MotionName.word_forward ? "word" : "WORD";
                ...
                case MotionName.word_tail: // fall through
                case MotionName.WORD_tail: {
                    // e/E motionは複数行にまたがることがある
                    const sep = motion.name === MotionName.word_tail ? "word" : "WORD";
